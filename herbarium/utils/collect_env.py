...
def collect_env_info():
    ...
    try:
        import herbarium  # noqa

        data.append(
            ("herbarium", herbarium.__version__ + " @" + os.path.dirname(herbarium.__file__))
        )
    except ImportError:
    ...
    try:
        ...
        data.append(("herbarium._C", f"not built correctly: {e}"))
        ...
        if has_cuda and sys.platform != "win32":
            try:
                so_file = importlib.util.find_spec("herbarium._C").origin
            except ImportError:
                pass
            else:
                data.append(
                    ("herbarium arch flags", detect_compute_compatibility(CUDA_HOME, so_file))
                )
    else:
...
if __name__ == "__main__":
    try:
        from herbarium.utils.collect_env import collect_env_info as f
